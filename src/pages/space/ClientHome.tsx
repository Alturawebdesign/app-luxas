...
export default function ClientHome({ client }: { client: Client }) {
  ...
  const kpis = [
    ...
    { icon: PhoneCall, label: 'Appels qualifiés / sem.', value: `${k.qualifiedCallsPerWeek}`, delta: 8, tint: 'bg-lime-100 text-lime-700' },
  ...
  return (
    <div className="animate-in space-y-6">
      ...
      <div className="card relative overflow-hidden !border-transparent p-0">
        <div className="absolute inset-0" style={{ background: 'linear-gradient(120deg, #047857 0%, #10B981 55%, #34D399 120%)' }} />
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={client.impressionsSeries} margin={{ left: -12, right: 8, top: 6, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="value" stroke="#10B981" strokeWidth={3} fill="url(#homeArea)" dot={false} activeDot={{ r: 5, strokeWidth: 2, stroke: '#fff' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="card flex items-center gap-4 p-6">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-lime-100 text-lime-700"><Flame size={22} /></span>
          <div>
